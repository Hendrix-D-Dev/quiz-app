...
export async function createQuiz(req, res) {
    try {
        ...
        if (!title || !text)
            return res.status(400).json({ error: "Missing quiz title or text" });
        ...
        const quiz = {
            ...
            description,
            questions,
            createdBy: req.verifiedUid || "anonymous",
            createdAt: Date.now(),
        };
...
export async function submitQuiz(req, res) {
    try {
        ...
        const uid = req.verifiedUid || payload.uid || "anonymous";
        const score = {
            quizId: id,
...
export async function submitGeneratedQuiz(req, res) {
    try {
...
export async function createRoomQuiz(req, res) {
    try {
        const { host, roomName, timeLimit, maxQuestions, roomCode, docText, difficulty = "normal" } = req.body;
